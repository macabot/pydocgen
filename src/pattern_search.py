...
def test_optimize():
    ...
    min_diff = 0.001
    max_iterations = 30
    (best_weights, best_score), _weight_scores = optimize([3.1, 4.1], get_score,
                                                         step_size,
                                                         min_diff,
                                                         max_iterations)
...
def main():
    ...
    d_args = DecodeArguments(input_file, output_file, language_model,
                 ...
                 stupid_backoff, n_size, nbest, empty_default, bleu_path,
                 reference)

    get_score = d_args.get_bleu_score

    optimize(feature_weights, get_score, args.step_size, args.min_score_diff,
             args.max_iterations)
